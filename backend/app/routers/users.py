...
def create_user_direct(payload: UserCreateDirect, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    check_domain(payload.email)
...
def invite_user(payload: InviteCreate, db: Session = Depends(get_db), current: User = Depends(require_admin)):
    ...
    invite = InviteToken(token=token, email=payload.email, role=payload.role, invited_by=current.id, expires_at=datetime.now(timezone.utc) + timedelta(days=7))
    db.add(invite)
    db.commit()
    return {"invite_token": token, "email": payload.email, "role": payload.role, "invite_link": f"/register?token={token}"}
...
def list_invites(db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    return [{"email": i.email, "role": i.role, "token": i.token, "expires_at": i.expires_at, "created_at": i.created_at} for i in invites]

@router.delete("/invites/{token}")
def delete_invite(token: str, db: Session = Depends(get_db), _: User = Depends(require_admin)):
...
@router.post("/invites/{token}/resend")
def resend_invite(token: str, db: Session = Depends(get_db), current: User = Depends(require_admin)):
    invite = db.query(InviteToken).filter(InviteToken.token == token, InviteToken.used == False).first()
    if not invite:
        raise HTTPException(status_code=404, detail="Invite not found")
    invite.expires_at = datetime.now(timezone.utc) + timedelta(days=7)
    db.commit()
    return {"token": token, "email": invite.email}

@router.post("/invite/accept", response_model=UserOut)
def accept_invite(payload: InviteAccept, db: Session = Depends(get_db)):
    invite = db.query(InviteToken).filter(InviteToken.token == payload.token, InviteToken.used == False).first()
    if not invite:
        raise HTTPException(status_code=400, detail="Invalid or expired invite token")
    if datetime.now(timezone.utc) > invite.expires_at.replace(tzinfo=timezone.utc):
        raise HTTPException(status_code=400, detail="Invite token has expired")
    user = User(email=invite.email, name=payload.name, hashed_password=hash_password(payload.password), role=invite.role)
    db.add(user)
    invite.used = True
    db.commit()
    db.refresh(user)
